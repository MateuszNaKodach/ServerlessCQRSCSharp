...
namespace HotelManagement.Reservations.Api.Functions
{
    public class ReservationsFunctions
    {
        ...
        [FunctionName("MakeReservationFunction")] //virtual workshop ex-9
        ...
        public async Task<IActionResult> CancelReservation([HttpTrigger(AuthorizationLevel.Function, "post", Route = "reservation/cancel")] HttpRequest req, CancellationToken cancellationToken, ILogger log)
        {
            try
            {
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
                var cancelReservationCommand = JsonConvert.DeserializeObject<CancelReservation>(requestBody);  //you may want to use a mapper from dto to command instead (backwards compatibility)
                //fire off command
                var events = await _bus.Send(cancelReservationCommand, cancellationToken);
                return new OkObjectResult(events);
            }
            catch (Exception ex)
            {
                log.LogCritical(ex, ex.Message, new Dictionary<string, object> { ["Request"] = req.Body });
                return new BadRequestObjectResult("Error occured on CancelReservationFunction.CancelReservation.");
            }
        }
    }
}
